...
data |>
  ...
  labs(x = "Field Position", 
       ...
       subtitle = "Forwards and Midfielders tend to earn more than Goalkeepers and Defenders in the MLS league in 2007",
       caption = "Source: Originally from MLS Union",
       color = "Position") + 
  theme_classic()
```

Player salaries are drastically different based on player positions in the MLS soccer league. This was sourced from a data set which used data originally from the MLS Union. Mapping data was then used to visualize the player salaries by field position in an intuitive format. Offensive player positions such as Midfield and Forwards on average earn more than defensive positions such as goalkeeper and defenders in the MLS league in 2007.
