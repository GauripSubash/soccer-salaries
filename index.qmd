...
Player salaries are drastically different based on player positions in the MLS soccer league. his was sourced from a data set which used data originally from the MLS Union. Mapping data was then used to visualize the player salaries by field position in an intuitive format.Player positions such as Midfield earn more than other defender positions such as Goalkeeper or defense in the MLS league from 2007.
...
mls_data |>
  ...
  labs(x = "Field Position", 
       y = "Base Salary", 
       title = "Highest Salary per position", 
       caption = "Source: KYLEAH FREDERIC") + 
  theme_classic()
```
